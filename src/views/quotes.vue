  ...
  <div class="flex h-screen bg-gray-100">
    ...
    <div class="flex-1 overflow-auto">
      <div class="p-8">
        ...
        <div class="space-y-6">
          <div v-for="(quote, index) in paginatedQuotes" :key="quote.id" class="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-400">
            ...
            <div class="flex justify-between items-end">
              <div>
                <p class="font-semibold text-gray-700">{{ quote.Book }}</p>
                <p class="text-sm text-gray-600">{{ quote.Author }}</p>
              </div>
              <p class="text-sm text-gray-500">{{ formatDate(quote.Datetime) }}</p>
            </div>
            <div class="mt-4 flex justify-end space-x-2">
              <button v-if="!quote.Editable" @click="quote.Editable = true" class="bg-yellow-500 hover:bg-yellow-600 text-white px-3 py-1 rounded">Edit</button>
              <button v-else @click="handleEdit(quote)" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded">Save</button>
              <button @click="handleDelete(quote.id)" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded">Delete</button>
            </div>
          </div>
        </div>
